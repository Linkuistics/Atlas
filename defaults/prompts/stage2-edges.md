# Discovery — Stage 2: Infer Cross-Component Edges

You are given a collection of per-component interaction-surface records.
...
2. **Source generation into another tree?** One component's
...
3. **Process spawning?** One component's `external_tools_spawned` names
...
4. **Library dependency?** Package-manifest entries, import
   ...
   → `depends-on` at the appropriate lifecycle (`build` for
     ...
     component expects the dependency to be present in the execution
     environment rather than bundled.
5. **In-process embedding?** One component runs the other in-process as
...
7. **Documentation relationship?** One component's purpose is to
   document another (docs repo, architecture notes, external user
   guide).
   → `describes @ design`.
8. **Test harness or fixture provider?** One component's primary
...
- **Shared upstream dependencies.** Two components independently
  ...
  `explicit_cross_component_mentions` is not evidence of an edge between
...
- **Generic or trivial file-glob overlap.** Patterns like `*.txt`,
  ...
  by one component and consumed by another.
