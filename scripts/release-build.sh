...
# formula from scripts/templates/atlas.rb.tmpl.
#
# Output: target/dist/
#   atlas-v<ver>-<target>.tar.xz   (one per target)
#   atlas.rb                       (rendered formula)
#
...
readonly TEMPLATE="$REPO_ROOT/scripts/templates/atlas.rb.tmpl"
...
build_target() {
  ...
  local stage="$DIST_DIR/staging/atlas-v${version}-${target}"
  mkdir -p "$stage"
  cp "$REPO_ROOT/target/$target/release/atlas" "$stage/atlas"
  cp "$REPO_ROOT/LICENSE" "$REPO_ROOT/README.md" "$stage/"

  local archive="$DIST_DIR/atlas-v${version}-${target}.tar.xz"
  tar -C "$DIST_DIR/staging" -cJf "$archive" "atlas-v${version}-${target}"
  echo "$archive"
}
