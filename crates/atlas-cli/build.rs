// Emit build-time metadata for the `--version` output so a user staring at a
// misbehaving binary can answer "which build is this?" without guessing from
// file mtimes.
//
...
fn main() {
    ...
    // Recompile the version metadata when HEAD moves or a tag is created —
    // otherwise `cargo build` would cache the first value and GIT_DESCRIBE
    // would stale out. Paths are relative to the crate manifest dir; the
    // workspace `.git` lives two levels up.
