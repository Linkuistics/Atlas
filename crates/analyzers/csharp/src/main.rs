//! `csharp-analyzer` subprocess entry point (Atlas vNext Phase 2 PR-6).
//!
...
//!    `id: "csharp-surface-analyzer"`, `stage: L5`,
//!    `cost_class: deterministic-expensive`, and the
//!    `applicability_predicate` keyed on `*.csproj` / `*.sln` manifest
//!    signals plus the `csharp` language tag.
//! 2. For each subsequent [`Request`] frame on stdin, parse it,
//!    drive [`atlas_csharp_analyzer::extract_csharp_surface`] when
//!    the request is `analyse`, and write the response frame on stdout.
//! 3. EOF on stdin → graceful exit.
//!
//! ## Source-file discovery
//!
//! The wire protocol's `Target.manifests` carries any pre-loaded
//! manifests (canonically `*.csproj`); it does NOT ship every `*.cs`
//! source file. The analyser walks the candidate dir at `Target.dir`
//! directly via `std::fs::read_dir` to enumerate C# sources, then
//! parses each. This mirrors the Python analyser's pattern.
...
use atlas_csharp_analyzer::{
    extract_csharp_surface, pub_item_kind_str, ANALYZER_ID, ANALYZER_VERSION,
};
    ...
    match request {
        ...
        Request::FingerprintInputs { target } => {
            let mut inputs: Vec<WireFingerprintInput> = target
                ...
                .collect();
            for (_rel, bytes) in walk_csharp_sources(Path::new(&target.dir)) {
...
fn analyse(target: &WireTarget) -> Response {
    ...
    let (csproj_bytes, csproj_name) = csproj_bytes_from_target(target);
    let sources = walk_csharp_sources(&dir);
    let inputs = atlas_csharp_analyzer::CsharpSourceInputs {
        sources,
        csproj: csproj_bytes,
        csproj_name,
    };
    let surface = extract_csharp_surface(&component_id, &inputs);
...
/// Component id heuristic: prefer `<AssemblyName>` from the csproj;
/// fall back to the dir basename.
fn component_id_from_target(target: &WireTarget) -> String {
    if let (Some(bytes), _) = csproj_bytes_from_target(target) {
        if let Ok(text) = std::str::from_utf8(&bytes) {
            if let Some(name) = atlas_csharp_analyzer::extract_csproj_assembly_name(text) {
                return name;
            }
        }
    }
...
/// Walk the candidate directory recursively, returning every `*.cs` file
/// as `(relative_path, bytes)`. Symlinks are not followed; files larger
/// than 4 MiB are skipped. The walk is depth-first and lexicographic.
fn walk_csharp_sources(dir: &Path) -> Vec<(PathBuf, Vec<u8>)> {
    const MAX_CS_FILE_BYTES: u64 = 4 * 1024 * 1024;
    ...
    while let Some(current) = stack.pop() {
        ...
        for entry in entries {
            ...
            if let Some(s) = basename.to_str() {
                if s.starts_with('.') || s == "bin" || s == "obj" || s == "node_modules" {
                    continue;
                }
            }
            ...
            if metadata.len() > MAX_CS_FILE_BYTES {
                continue;
            }
