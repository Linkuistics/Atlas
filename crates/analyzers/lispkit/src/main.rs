//! `lispkit-analyzer` subprocess entry point (Atlas vNext Phase 2 PR-10).
//!
...
//!    `id: "lispkit-surface-analyzer"`, `stage: L5`,
//!    `cost_class: deterministic-expensive`, and the
//!    `applicability_predicate` keyed on the LispKit language tag
//!    plus `**/*.sld` glob.
//! 2. For each subsequent [`Request`] frame on stdin, parse it,
//!    drive [`atlas_lispkit_analyzer::extract_lispkit_surface`] when
...
//!
//! The analyser walks `Target.dir` directly via `std::fs::read_dir`
//! to enumerate `*.sld` files, then parses each. This mirrors the
//! Python analyser's walk-at-runtime approach (Phase 2 minimum).
...
use atlas_lispkit_analyzer::{
    extract_lispkit_surface, pub_item_kind_str, LispKitSourceInputs, ANALYZER_ID, ANALYZER_VERSION,
};

/// Wire-side representation of one [`atlas_index::Binding`]. Keeps the
/// on-wire shape decoupled from the in-process struct.
    ...
    match request {
        ...
        Request::FingerprintInputs { target } => {
            let mut inputs: Vec<WireFingerprintInput> = target
                ...
                .collect();
            for (_rel, bytes) in walk_sld_sources(Path::new(&target.dir)) {
...
fn analyse(target: &WireTarget) -> Response {
    ...
    let sources = walk_sld_sources(&dir);
    let inputs = LispKitSourceInputs { sources };
    let surface = extract_lispkit_surface(&component_id, &inputs);
...
/// Derive a component id from the wire target. Falls back to the dir
/// basename if no known manifest names the component.
fn component_id_from_target(target: &WireTarget) -> String {
    // For LispKit components the canonical id is the directory basename.
    // A future driver may extract the library identifier from the first
    // `*.sld` file found, but the directory name is the safe fallback.
    target
        .manifests
        .iter()
        .find(|m| m.name.ends_with(".sld"))
        .and_then(|_| {
            // The dir basename is the conventional id.
            Path::new(&target.dir)
                .file_name()
                .and_then(|n| n.to_str())
                .map(str::to_string)
...
/// Walk the candidate directory recursively, returning every `*.sld`
/// file as `(relative_path, bytes)`. Symlinks are not followed; files
/// larger than 4 MiB are skipped. The walk is lexicographic so output
/// ordering is stable.
fn walk_sld_sources(dir: &Path) -> Vec<(PathBuf, Vec<u8>)> {
    const MAX_SLD_FILE_BYTES: u64 = 4 * 1024 * 1024;
    ...
    while let Some(current) = stack.pop() {
        ...
        for entry in entries {
            ...
            if metadata.len() > MAX_SLD_FILE_BYTES {
                continue;
            }
            let extension = path.extension().and_then(OsStr::to_str);
            if !matches!(extension, Some("sld")) {
                continue;
            }
