//! `racket-analyzer` subprocess entry point (Atlas vNext Phase 2 PR-9).
//!
...
//!    `id: "racket-surface-analyzer"`, `stage: L5`,
//!    `cost_class: deterministic-expensive`, and the
//!    `applicability_predicate` keyed on the Racket language tag
//!    plus `info.rkt`.
//! 2. For each subsequent [`Request`] frame on stdin, parse it,
//!    drive [`atlas_racket_analyzer::extract_racket_surface`] when
...
//!
//! The subprocess does NOT have LLM access (per PR-2 / PR-9 charter
//! in the plan). All extraction is structural, driven by the
//! hand-rolled minimal s-expression reader over the pre-loaded Racket
//! source bytes.
//!
//! ## Source-file discovery
//!
//! The wire protocol's `Target.manifests` only carries the manifests
//! the parent pre-loaded (canonically `info.rkt`); it does NOT ship
//! every `*.rkt` source file in the component. The analyser walks the
//! candidate dir at `Target.dir` directly via `std::fs::read_dir` to
//! enumerate Racket sources, then parses each. This matches the Python
//! analyser's pattern exactly.
...
use atlas_racket_analyzer::{
    extract_racket_surface, pub_item_kind_str, sha256_hex_bytes, ANALYZER_ID, ANALYZER_VERSION,
};
...
fn handle_request(request: &Request) -> Response {
    match request {
        ...
        Request::FingerprintInputs { target } => {
            // Contributors:
            // - Every pre-loaded manifest's content_sha (info.rkt).
            // - Every Racket source file's content_sha discovered
            //   under `target.dir`.
            let mut inputs: Vec<WireFingerprintInput> = target
                ...
                .collect();
            for (_rel, bytes) in walk_racket_sources(Path::new(&target.dir)) {
...
fn analyse(target: &WireTarget) -> Response {
    ...
    let info_rkt = info_rkt_bytes_from_target(target);
    let sources = walk_racket_sources(&dir);
    let inputs = atlas_racket_analyzer::RacketSourceInputs { sources, info_rkt };
    let surface = extract_racket_surface(&component_id, &inputs);
...
/// Extract the `info.rkt` bytes from the parent's pre-loaded manifest
/// list. The `bytes_b64` field is base64; we decode here.
fn info_rkt_bytes_from_target(target: &WireTarget) -> Option<Vec<u8>> {
    target
        .manifests
        .iter()
        .find(|m| m.name == "info.rkt")
        .and_then(|m| BASE64.decode(&m.bytes_b64).ok())
}

/// Component id heuristic for the analyse path: use the dir basename.
/// The actual canonical id is set by the parent's L4 walk; this string
/// only flows into `LibraryApi.id` (`<id>/public-api`) and is
/// overwritten if the parent wishes to canonicalise.
...
/// Walk the candidate directory recursively, returning every `*.rkt`
/// file as `(relative_path, bytes)`. Symlinks are not followed; files
/// larger than 4 MiB are skipped. The walk is depth-first and
/// lexicographic so output ordering is stable across runs.
fn walk_racket_sources(dir: &Path) -> Vec<(PathBuf, Vec<u8>)> {
    const MAX_RKT_FILE_BYTES: u64 = 4 * 1024 * 1024;
    ...
    while let Some(current) = stack.pop() {
        ...
        for entry in entries {
            ...
            if metadata.len() > MAX_RKT_FILE_BYTES {
                continue;
            }
            let extension = path.extension().and_then(OsStr::to_str);
            if extension != Some("rkt") {
                continue;
            }
