...
- `subcarve-with-tools.jsonl` — prompt that requires reading one file; assistant emits a `tool_use` (`Read` with `file_path` input); the next user turn carries a `tool_result` (with **`is_error` absent**, the success encoding); terminal `result.subtype = "success"`. Exercises `BackendCallEvent::ToolUse`/`ToolResult` extraction.
