//! Pass-through wrapper for the Dart subprocess surface analyser.
//!
//! Wraps `atlas_analyzers::cached_dart_subprocess_proxy` in a `Tool`. The
//! subprocess payload is forwarded verbatim as the `ToolResult` output. When
//! the `dart-analyzer` binary is not found in the cargo target tree, returns a
//! descriptive `ToolError::Invocation`.
...
/// Pass-through tool wrapping the Dart surface subprocess analyser.
pub struct DartSurfaceTool;
    ...
    SCHEMA.get_or_init(|| ToolSchema {
        args_schema: serde_json::json!({
            ...
            "properties": {
                "component_dir": {
                    "type": "string",
                    "description": "workspace-relative path to the Dart component directory"
                },
                "manifest_path": {
                    "type": "string",
                    "description": "optional workspace-relative path to pubspec.yaml"
                }
            },
            "required": ["component_dir"]
        }),
        description:
            "Extract the Dart surface (bindings, library APIs, path-dep edges) from a Dart \
             component via the dart-analyzer subprocess. Returns the raw analyser JSON payload. \
             Requires the dart-analyzer binary to be built (`cargo build -p atlas-dart-analyzer`)."
                .into(),
    })
}

#[async_trait]
impl Tool for DartSurfaceTool {
    fn id(&self) -> &'static str {
        "extract_dart_surface"
    }
    ...
    async fn invoke(&self, args: ToolArgs, ctx: &ToolContext) -> Result<ToolResult, ToolError> {
        ...
        let output =
            tokio::task::spawn_blocking(move || -> Result<serde_json::Value, ToolError> {
                ...
                let binary_path = locate_dart_analyzer_binary().ok_or_else(|| {
                    ToolError::Invocation(
                        "dart-analyzer binary not found in cargo target tree; \
                     run `cargo build -p atlas-dart-analyzer` first"
                            .into(),
                    )
                })?;

                let spec = dart_subprocess_spec(binary_path);
                let proxy = cached_dart_subprocess_proxy(spec).map_err(|e| {
                    ToolError::Invocation(format!("failed to construct dart-analyzer proxy: {e}"))
                })?;
                ...
                match proxy.analyse(&ctx, &target) {
                    ...
                    AnalyzerResult::Declines => Err(ToolError::Invocation(
                        "dart-analyzer declined this target".into(),
                    )),
                    AnalyzerResult::Error(e) => {
                        Err(ToolError::Invocation(format!("dart-analyzer error: {e}")))
                    }
                }
            })
            .await
...
mod tests {
    ...
    #[tokio::test]
    async fn dart_surface_tool_either_succeeds_or_returns_helpful_error() {
