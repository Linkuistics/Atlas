//! Pass-through wrapper for the Python subprocess surface analyser.
//!
//! Wraps `atlas_analyzers::cached_subprocess_proxy` (the Python one) in a
//! `Tool`. The subprocess payload is forwarded verbatim as the `ToolResult`
//! output — no re-parsing or re-shaping. When the `python-analyzer` binary
//! is not found in the cargo target tree, returns a descriptive
...
/// Pass-through tool wrapping the Python surface subprocess analyser.
pub struct PythonSurfaceTool;
    ...
    SCHEMA.get_or_init(|| ToolSchema {
        args_schema: serde_json::json!({
            ...
            "properties": {
                "component_dir": {
                    "type": "string",
                    "description": "workspace-relative path to the Python component directory"
                },
                "manifest_path": {
                    "type": "string",
                    "description": "optional workspace-relative path to pyproject.toml, setup.py, \
                                    or requirements.txt (used to populate the manifest list)"
                }
            },
            "required": ["component_dir"]
        }),
        description:
            "Extract the Python surface (bindings, library APIs, path-dep edges) from a Python \
             component via the python-analyzer subprocess. Returns the raw analyser JSON payload. \
             Requires the python-analyzer binary to be built (`cargo build -p atlas-python-analyzer`)."
                .into(),
    })
}

#[async_trait]
impl Tool for PythonSurfaceTool {
    ...
    async fn invoke(&self, args: ToolArgs, ctx: &ToolContext) -> Result<ToolResult, ToolError> {
        ...
        let output =
            tokio::task::spawn_blocking(move || -> Result<serde_json::Value, ToolError> {
                ...
                let binary_path = locate_python_analyzer_binary().ok_or_else(|| {
                    ToolError::Invocation(
                        "python-analyzer binary not found in cargo target tree; \
                     run `cargo build -p atlas-python-analyzer` first"
                            .into(),
                    )
                })?;

                let spec = python_subprocess_spec(binary_path);
                let proxy = cached_subprocess_proxy(spec).map_err(|e| {
                    ToolError::Invocation(format!("failed to construct python-analyzer proxy: {e}"))
                })?;
                ...
                match proxy.analyse(&ctx, &target) {
                    ...
                    AnalyzerResult::Declines => Err(ToolError::Invocation(
                        "python-analyzer declined this target".into(),
                    )),
                    AnalyzerResult::Error(e) => {
                        Err(ToolError::Invocation(format!("python-analyzer error: {e}")))
                    }
                }
            })
            .await
    ...
    fn fingerprint_inputs(&self, _args: &ToolArgs) -> Vec<FingerprintInput> {
        // Subprocess analysers walk the component directory internally;
        // the parent cannot enumerate the exact input set without running
        // the subprocess. Return empty — the runtime treats this as
        // "inputs unknown; do not cache".
        vec![]
    }
}
...
mod tests {
    ...
    #[tokio::test]
    async fn python_surface_tool_either_succeeds_or_returns_helpful_error() {
