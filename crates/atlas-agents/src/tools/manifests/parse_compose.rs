//! Pass-through wrapper for `atlas_analyzers::compose_classifier::parse_compose`.
    ...
    SCHEMA.get_or_init(|| ToolSchema {
        args_schema: serde_json::json!({
            ...
            "properties": {
                "path": {
                    "type": "string",
                    "description": "workspace-relative path to a docker-compose.yml or compose.yml"
                }
            },
            "required": ["path"]
        }),
        description: "Parse a Docker Compose file. Returns the list of declared services with \
                      their image references and build contexts."
            .into(),
    })
}

#[async_trait]
impl Tool for ParseComposeTool {
    ...
    async fn invoke(&self, args: ToolArgs, ctx: &ToolContext) -> Result<ToolResult, ToolError> {
        ...
        let output = tokio::task::spawn_blocking({
            ...
            move || -> Result<serde_json::Value, ToolError> {
                ...
                let shape = atlas_analyzers::compose_classifier::parse_compose(&contents);
                serde_json::to_value(&shape)
                    .map_err(|e| ToolError::Invocation(format!("serialize ComposeShape: {e}")))
            }
        })
        .await
...
mod tests {
    ...
    #[tokio::test]
    async fn parse_compose_tool_matches_direct_call() {
        let tempdir = tempfile::TempDir::new().unwrap();
        let content = "services:\n  web:\n    image: nginx:latest\n  db:\n    image: postgres:16\n";
        std::fs::write(tempdir.path().join("docker-compose.yml"), content).unwrap();

        // Direct call
        let direct = atlas_analyzers::compose_classifier::parse_compose(content);
        let expected = serde_json::to_value(&direct).unwrap();

        // Wrapper call
        let tool = ParseComposeTool;
        let args = ToolArgs(serde_json::json!({ "path": "docker-compose.yml" }));
        let ctx = ToolContext {
