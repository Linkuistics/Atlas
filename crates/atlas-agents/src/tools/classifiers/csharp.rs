//! Pass-through wrapper for `atlas_analyzers::csharp_classifier::CsharpClassifier`.
//!
//! Exposes the C# L3 classifier as a `Tool` so the LLM-spine runtime can
//! invoke it directly. No new logic — pure pass-through.

use async_trait::async_trait;
use atlas_analyzers::{
    AnalysisContext, Analyzer, AnalyzerResult, CsharpClassificationOutput, CsharpClassifier,
    Target, TargetFile,
};
...
/// Pass-through tool wrapping [`CsharpClassifier`].
pub struct CsharpClassifyTool;
    ...
    SCHEMA.get_or_init(|| ToolSchema {
        args_schema: serde_json::json!({
            ...
            "properties": {
                ...
                "manifest_path": {
                    "type": "string",
                    "description": "workspace-relative path to *.csproj or *.sln manifest"
                }
            },
            "required": ["component_dir", "manifest_path"]
        }),
        description: "Classify a C# component. Returns kind (csharp-project or csharp-solution), \
             build system (msbuild), evidence fields, and rationale. Pass the manifest \
             file (*.csproj or *.sln) as `manifest_path`."
            .into(),
    })
}

#[async_trait]
impl Tool for CsharpClassifyTool {
    fn id(&self) -> &'static str {
        "classify_csharp_component"
    }
    ...
    async fn invoke(&self, args: ToolArgs, ctx: &ToolContext) -> Result<ToolResult, ToolError> {
        ...
        let output = tokio::task::spawn_blocking(
            move || -> Result<CsharpClassificationOutput, ToolError> {
                ...
                let analyser = CsharpClassifier::new();
                ...
                match analyser.analyse(&ctx, &target) {
                    ...
                    AnalyzerResult::Declines => Err(ToolError::Invocation(
                        "csharp classifier declined: no *.csproj or *.sln manifest found".into(),
                    )),
...
mod tests {
    ...
    #[tokio::test]
    async fn csharp_classify_tool_matches_direct_call_csproj() {
        let tempdir = tempfile::TempDir::new().unwrap();
        let manifest_body = "<Project Sdk=\"Microsoft.NET.Sdk\">\n  <PropertyGroup>\n    <TargetFramework>net8.0</TargetFramework>\n  </PropertyGroup>\n</Project>\n";
        std::fs::write(tempdir.path().join("MyApp.csproj"), manifest_body).unwrap();
        ...
        let target = Target {
            ...
            languages: BTreeSet::from(["csharp".to_string()]),
            manifests: vec![TargetFile {
                name: "MyApp.csproj".into(),
                relpath: PathBuf::from("MyApp.csproj"),
                bytes,
                content_sha,
            }],
            top_level_files: vec![],
        };
        let direct: CsharpClassificationOutput = match CsharpClassifier::new()
            .analyse(&AnalysisContext::deterministic_only(), &target)
        {
            AnalyzerResult::Confident(boxed) => boxed
                .as_any()
                .downcast_ref::<CsharpClassificationOutput>()
    ...
    #[tokio::test]
    async fn csharp_classify_tool_returns_error_when_manifest_missing() {
        let tempdir = tempfile::TempDir::new().unwrap();
        let tool = CsharpClassifyTool;
        let args = ToolArgs(serde_json::json!({
            "component_dir": "",
            "manifest_path": "NonExistent.csproj"
        }));
        let ctx = ToolContext {
            workspace_root: tempdir.path().to_path_buf(),
        };
