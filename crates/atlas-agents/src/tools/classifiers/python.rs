//! Pass-through wrapper for `atlas_analyzers::python_classifier::PythonClassifier`.
//!
//! Exposes the Python L3 classifier as a `Tool` so the LLM-spine runtime
//! can invoke it directly. No new logic — pure pass-through.

use async_trait::async_trait;
use atlas_analyzers::{
    AnalysisContext, Analyzer, AnalyzerResult, PythonClassificationOutput, PythonClassifier,
    Target, TargetFile,
};
...
/// Pass-through tool wrapping [`PythonClassifier`].
pub struct PythonClassifyTool;
    ...
    SCHEMA.get_or_init(|| ToolSchema {
        args_schema: serde_json::json!({
            ...
            "properties": {
                ...
                "manifest_path": {
                    "type": "string",
                    "description": "workspace-relative path to pyproject.toml, setup.py, or requirements.txt"
                }
            },
            "required": ["component_dir", "manifest_path"]
        }),
        description:
            "Classify a Python component. Returns kind (python-package), evidence fields, \
             build system, and rationale. Pass the manifest file (pyproject.toml, setup.py, \
             or requirements.txt) as `manifest_path`."
                .into(),
    })
}

#[async_trait]
impl Tool for PythonClassifyTool {
    fn id(&self) -> &'static str {
        "classify_python_component"
    }
    ...
    async fn invoke(&self, args: ToolArgs, ctx: &ToolContext) -> Result<ToolResult, ToolError> {
        ...
        let output = tokio::task::spawn_blocking(
            move || -> Result<PythonClassificationOutput, ToolError> {
                ...
                let analyser = PythonClassifier::new();
                ...
                match analyser.analyse(&ctx, &target) {
                    ...
                    AnalyzerResult::Declines => Err(ToolError::Invocation(
                        "python classifier declined: no recognised manifest".into(),
                    )),
...
mod tests {
    ...
    #[tokio::test]
    async fn python_classify_tool_matches_direct_call() {
        let tempdir = tempfile::TempDir::new().unwrap();
        let manifest_body = "[project]\nname=\"foo\"\nversion=\"0.1.0\"\n";
        std::fs::write(tempdir.path().join("pyproject.toml"), manifest_body).unwrap();
        ...
        let direct: PythonClassificationOutput = match PythonClassifier::new()
    ...
    #[tokio::test]
    async fn python_classify_tool_returns_error_when_manifest_missing() {
        let tempdir = tempfile::TempDir::new().unwrap();
        let tool = PythonClassifyTool;
        let args = ToolArgs(serde_json::json!({
            "component_dir": "",
